...
/**
 * Internal intermediary structure derived from FFooDistortionCameraModel by the game thread
...
struct FCompiledCameraModel
{
	...
	FFooDistortionCameraModel OriginalCameraModel;
...
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationVS, TEXT("/FooShaderDir/Private/MyShader.usf"), TEXT("MainVS"), SF_Vertex)
IMPLEMENT_SHADER_TYPE(, FLensDistortionUVGenerationPS, TEXT("/FooShaderDir/Private/MyShader.usf"), TEXT("MainPS"), SF_Pixel)
...
FVector2D FFooDistortionCameraModel::UndistortNormalizedViewPosition(FVector2D EngineV) const
{
...
float FFooDistortionCameraModel::GetUndistortOverscanFactor(
	float DistortedHorizontalFOV, float DistortedAspectRatio) const
{
	...
	if (*this == FFooDistortionCameraModel())
	{
		return 1.0f;
	}
...
void FFooDistortionCameraModel::DrawUVDisplacementToRenderTarget(
	UWorld* World,
